...
## Repository contents
- `arduino/arduino` — **Main firmware**: IMU + complementary filter + X-gesture + stepper homing/center + button push + CSV stream  
- `processing/digital_twin.pde` — **Digital Twin UI**: serial CSV parsing + 3D board + live plots + handshake
- `gesture+digital_twin.mp4` — **Video**
...
### Run (Arduino)
1. Open `arduino/arduino.ino` and **upload** to Arduino Uno.
